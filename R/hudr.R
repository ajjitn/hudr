...
get_fmrdata_info <- function(X) {
  X[["data_info"]] <- data.table("county_name" = X$county_name,
  ...
  return(X[c("data_info", "basicdata")])
}
...
#'                                 hud_key = hud_key)
#' }
get_hud_fmr_data <- function(entityid = "METRO10180M10180", year, hud_key) {
  pass <- "PASS"
  pass[] <- check_args(pass = pass, argument = entityid, label = "entityid")
  pass[] <- check_args(pass = pass, argument = year, label = "year")
  if (pass != "PASS") {
    stop(pass)
  }
  dta <- tryCatch(expr = {
    url <- paste0("https://www.huduser.gov/hudapi/public/fmr/data/", entityid, "?year=", year)
    get_response <- httr::GET(url = url, config = add_headers("Authorization" = paste0("Bearer ", hud_key)))
    get_content <- httr::content(x = get_response, simplifyVector = FALSE)[["data"]] %>% get_fmrdata_info() %>%
      lapply(FUN = function(X, entityid, year) {as.data.table(x = X)[, c("entityid", "year") := list(entityid, year)]}, entityid = entityid, year = year) %>%
...
#'                                       hud_key = hud_key)
#' }
get_hud_fmr_statedata <- function(entityid = "AL", year, hud_key) {
  pass <- "PASS"
  pass[] <- check_args(pass = pass, argument = entityid, label = "entityid")
  pass[] <- check_args(pass = pass, argument = year, label = "year")
  if (pass != "PASS") {
  ...
  dta <- tryCatch(expr = {
    url <- paste0("https://www.huduser.gov/hudapi/public/fmr/statedata/", entityid, "?year=", year)
    get_response <- httr::GET(url = url, config = add_headers("Authorization" = paste0("Bearer ", hud_key)))
    get_content <- httr::content(x = get_response, simplifyVector = TRUE)[["data"]][c("metroareas", "counties")] %>%
      lapply(FUN = function(X, entityid, year) {as.data.table(x = X)[, c("state_code", "year") := list(entityid, year)]}, entityid = entityid, year = year) %>%
      lapply(FUN = janitor::clean_names)
...
#'                               hud_key = hud_key)
#' }
get_hud_il_data <- function(entityid = "0100199999", year, hud_key) {
  pass <- "PASS"
  pass[] <- check_args(pass = pass, argument = entityid, label = "entityid")
  pass[] <- check_args(pass = pass, argument = year, label = "year")
  if (pass != "PASS") {
  ...
  dta <- tryCatch(expr = {
    url <- paste0("https://www.huduser.gov/hudapi/public/il/data/", entityid, "?year=", year)
    get_response <- httr::GET(url = url, config = add_headers("Authorization" = paste0("Bearer ", hud_key)))
...
#'                                     hud_key = hud_key)
#' }
get_hud_il_statedata <- function(entityid = "AL", year, hud_key) {
  pass <- "PASS"
  pass[] <- check_args(pass = pass, argument = entityid, label = "entityid")
  pass[] <- check_args(pass = pass, argument = year, label = "year")
  if (pass != "PASS") {
  ...
  dta <- tryCatch(expr = {
    url <- paste0("https://www.huduser.gov/hudapi/public/il/statedata/", entityid, "?year=", year)
    get_response <- httr::GET(url = url, config = add_headers("Authorization" = paste0("Bearer ", hud_key)))
